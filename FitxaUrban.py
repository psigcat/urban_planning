...
/***************************************************************************
 FitxaUrban 2.0
                                 A QGIS plugin
 Generació fitxes urbanístiques
...
class FitxaUrban:
    ...
    def Preparar(self): #Preparacio d'inici o canvi de projecte qgs

        self.CONFIG = self.llegirConfig()
        if self.PREPAR == "NO" : return
        ...
        if self.dir_pdfs == "" : self.dir_pdfs = os.path.join(self.project_folder, 'pdfs')
        ...
        if self.dir_html == "" : self.dir_html = os.path.join(self.project_folder, 'html')
        self.dir_sector = self.Config("DIR_SEC")
        if self.dir_sector == "" : self.dir_sector = os.path.join(self.dir_html, 'sectors')
        self.dir_classi = self.Config("DIR_CLAS")
        if self.dir_classi == "" : self.dir_classi = os.path.join(self.dir_html, 'classificacio')
        self.dir_orden = self.Config("DIR_ORD")
        if self.dir_orden == "" : self.dir_orden = os.path.join(self.dir_html, 'ordenacions')
        ...
        else : db.setConnectOptions("service="+self.Config("BD_SERVICE"))
        db.open()
        if db.isOpen() == 0 :
    ...
    def llegirConfig(self):
        ...
        for reg in f :
            if len(reg) > 5 :
                if reg[0] != "#" and reg.find(" = ") != -1 : conf +=reg.strip()+"\n"
        f.close()
        return conf
        
    def Config(self,param):
        if self.CONFIG.find(param+" = ") == -1 : return ""
        return self.CONFIG.split(param+" = ")[1].split("\n")[0].strip()
    ...
    def activateTool(self):
        ...
        k = ""
        if self.CONFIG.find("LAYER_NAME"+" = ") != -1 : k = self.CONFIG.split("LAYER_NAME"+" = ")[1].split("\n")[0]
        if k == "" :
    ...
    def unload(self):
        ...
        if self.BD_OPEN == "SI" :
            ...
            self.PREPAR = "NO"
        self.action.setChecked(False)
        if self.dialog : self.tancaGui()
        self.iface.removePluginMenu(u'FitxaUrban', self.action)
        self.iface.removeToolBarIcon(self.action)
        
    def Missatge(self,av,t):    #Finestra missatges
            m = QMessageBox()
            if av == "W" :
    ...
    def run(self):
        ...
        if len(features) < 1 : 
            ...
            return
        if len(features) > 1 : layer.selectByIds([features[0].id()])
        feature = features[0]
        ...
        if self.dtop!= 0 and self.dleft != 0 : self.dialog.setGeometry(self.dleft,self.dtop,self.dialog.width(),self.dialog.height())
        ...
        for i in range(0, 4) :
            ...
            try:
                ...
                if os.path.isfile(filepath) : zz = u"<a href='file:///{:s}'>{:s}</a>".format(filepath, filename)
                else : zz = self.general_codes.split(",")[i]
        ...
        def makeShowUbicacioPdf():
            ...
            try: self.iface.removeMapLayers(self.Config("SELEC_NAME"))
            except: a=1
            ...
            if k.strip() == "" : k=os.path.join(self.plugin_dir,"FitxaUrban.qml")
            vl.loadNamedStyle(k)
            ...
            def refreshed():
                ...
                # Make PDF
                nl = ""
                if self.Config("DIR_PDFS_MULTI") == "SI" : nl = self.tr(socket.gethostname()+"("+time.strftime("%d")+")_")
        ...
        def makeShowZonesPdf():
            global db
            qu = QSqlQuery(db) 
            sq = self.SQL_FITXA_ZONA.split("$ID_VALUE")[0]+str(self.id_selec)+self.SQL_FITXA_ZONA.split("$ID_VALUE")[1]
            if qu.exec_(sq) == 0 :
            ...
            while (qu.next()) : 
                if qu.value(per_int) >= per_min :
                    ...
                    if composition is None :
                        self.Missatge("C","No s'ha trobat plantilla fitxa en el projecte")
                        return
                    for i in range(qu.record().count()) :
                    ...
                    exporter = QgsLayoutExporter(composition)                
                    exporter.exportToPdf(filename,QgsLayoutExporter.PdfExportSettings())                    
                    if self.Config("PDF_ZONES_VISU") != "1" : openFile(filename)
                    lispdfs.append(filename)
            qu.clear()
            merger = PdfFileMerger()
            for file in lispdfs : merger.append(PdfFileReader(file))
            nl = ""
            if self.Config("DIR_PDFS_MULTI") == "SI" : nl = self.tr(socket.gethostname()+"("+time.strftime("%d")+")_")
            filename = os.path.join(self.dir_pdfs, '{}{}_zones.pdf'.format(nl,self.refcat))
            merger.write(filename)
            if self.Config("PDF_ZONES_VISU") != "2" : openFile(filename)
